...
OSA_TASK_DEFINE(timerLED_Task,mAppTaskPrio_c, 1, mAppStackSize_c, FALSE);
...
void RGB_Timeout()
{
	OSA_EventSet(LEDEvent, gTimerLED_Event);
}

void timerLED_Task_init(void)
{
...
{
    /* start the timer */
    if(!TMR_IsTimerActive(LEDTimer))
    {
      TMR_StartIntervalTimer(LEDTimer, 2000, (pfTmrCallBack_t)RGB_Timeout, (void*)((uint32_t)LEDTimer));
    }
}
